...
def test_ready_tasks_empty_dependencies(data: Any, num_tasks: int) -> None:
    ...
    for _ in range(num_tasks):
        task_status = data.draw(
            st.sampled_from([Status.NOT_STARTED, Status.IN_PROGRESS, Status.BLOCKED])
        )
        task = data.draw(
            task_strategy(task_list_id=task_list_id, dependencies=[], status=task_status)
        )
        tasks.append(task)
    ...
    for _ in range(num_task_lists):
        ...
        for _ in range(num_tasks):
            ...
            if has_dependencies:
                ...
                task = data.draw(
                    task_strategy(
                        task_list_id=task_list_id, dependencies=[dependency], status=task_status
                    )
                )
...
def test_ready_tasks_mixed_dependencies(data: Any) -> None:
    ...
    dependent_task = data.draw(
        task_strategy(task_list_id=task_list_id, dependencies=dependencies, status=task_status)
    )
    ...
    ready_task_ids = {task.id for task in ready_tasks}
    assert (
        completed_dep.id not in ready_task_ids
    ), "COMPLETED task should NOT be ready (already done)"
    assert (

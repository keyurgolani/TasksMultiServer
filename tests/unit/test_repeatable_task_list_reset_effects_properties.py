...
def test_reset_clears_execution_notes(
    ...
    try:
        ...
        for i in range(num_tasks):
            execution_notes = [
                Note(f"Execution note {j}", datetime.now(UTC)) for j in range(num_execution_notes)
            ]
...
def test_reset_preserves_task_structure_and_other_fields(
    ...
    try:
        ...
        task_data_before = []
        for i in range(num_tasks):
            notes = [Note(f"Note {j}", datetime.now(UTC)) for j in range(num_notes)]
            research_notes = (
                [Note(f"Research {j}", datetime.now(UTC)) for j in range(num_research_notes)]
            ...
            task = Task(
                ...
                execution_notes=[Note("Execution", datetime.now(UTC))],
    ...
    try:
        ...
        for i in range(num_tasks):
            task = Task(
                ...
                priority=Priority.MEDIUM,
                notes=[Note(f"Note {i}", datetime.now(UTC))],
                research_notes=[Note(f"Research {i}", datetime.now(UTC))],
                action_plan=[ActionPlanItem(0, f"Action {i}")],
                execution_notes=[Note(f"Execution {i}", datetime.now(UTC))],

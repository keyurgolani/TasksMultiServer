...
class TestFileCreationAndAtomicWrites:
    ...
    def test_atomic_write_with_complex_task_data(
        ...
        sample_task.research_notes = [
            Note(content="Research note 1", timestamp=datetime.now(timezone.utc)),
            Note(content="Research note 2", timestamp=datetime.now(timezone.utc)),
        ]
        ...
        sample_task.execution_notes = [
            Note(content="Execution note 1", timestamp=datetime.now(timezone.utc)),
        ]
...
class TestFileLockingBehavior:
    ...
    def test_concurrent_project_creation(self, temp_store):
        ...
        assert (
            len(successful) >= 7
        ), f"Expected at least 7 successful creations, got {len(successful)}"

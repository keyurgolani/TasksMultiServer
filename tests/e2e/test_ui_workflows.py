...
class TestTaskWorkflow:
    ...
    def test_create_view_edit_delete_task(self, api_client, wait_for_api):
        ...
        task_list_data = {"name": f"E2E Task Test List {unique_id}", "project_name": f"E2E Task Test Project {unique_id}"}
        response = api_client.post(f"{API_BASE_URL}/task-lists", json=task_list_data)
        response_data = response.json()
        task_list_id = response_data["task_list"]["id"]
    ...
    def test_task_with_dependencies(self, api_client, wait_for_api):
        ...
        task_list_data = {"name": f"E2E Dependency Test List {unique_id}", "project_name": f"E2E Dependency Test Project {unique_id}"}
        response = api_client.post(f"{API_BASE_URL}/task-lists", json=task_list_data)
        response_data = response.json()
        task_list_id = response_data["task_list"]["id"]
...
class TestErrorHandling:
    ...
    def test_validation_errors(self, api_client, wait_for_api):
        ...
        task_list_data = {"name": f"E2E Error Test List {unique_id}", "project_name": f"E2E Error Test Project {unique_id}"}
        response = api_client.post(f"{API_BASE_URL}/task-lists", json=task_list_data)
        response_data = response.json()
        task_list_id = response_data["task_list"]["id"]
    ...
    def test_business_logic_errors(self, api_client, wait_for_api):
        ...
        task_list_data = {"name": f"E2E Business Logic Test List {unique_id}", "project_name": f"E2E Business Logic Test Project {unique_id}"}
        response = api_client.post(f"{API_BASE_URL}/task-lists", json=task_list_data)
        response_data = response.json()
        task_list_id = response_data["task_list"]["id"]
...
class TestCompleteUserJourney:
    ...
    def test_complete_project_lifecycle(self, api_client, wait_for_api):
        ...
        try:
            # Step 2: Create a task list
            task_list_data = {"name": f"Sprint 1 {unique_id}", "project_name": f"Complete Journey Project {unique_id}"}
            response = api_client.post(f"{API_BASE_URL}/task-lists", json=task_list_data)
            assert response.status_code == 200
            response_data = response.json()
            task_list_id = response_data["task_list"]["id"]

...
import type { ActionPlanItem as ActionPlanItemType, ExitCriterion, Task } from '../api/client';
...
export const CreateTaskModal: React.FC<CreateTaskModalProps> = ({ 
  ...
  const [notes, setNotes] = useState<{ type: 'general' | 'research' | 'execution'; content: string }[]>([]);
    ...
    try {
      const response = await fetch('http://localhost:8000/tasks', {
        ...
        body: JSON.stringify({
          ...
          notes: notes.filter(n => n.type === 'general').map((n, i) => ({ sequence: i + 1, content: n.content })),
          research_notes: notes.filter(n => n.type === 'research').map((n, i) => ({ sequence: i + 1, content: n.content })),
          execution_notes: notes.filter(n => n.type === 'execution').map((n, i) => ({ sequence: i + 1, content: n.content })),
        })
      });

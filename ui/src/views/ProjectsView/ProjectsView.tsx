 ...
 * Returns string for field to allow custom sort fields like "taskListCount"
 */
function parseSortOption(sortId: string): { field: string; direction: SortDirection } {
  const [field, direction] = sortId.split("-") as [string, SortDirection];
...
export const ProjectsView: React.FC<ProjectsViewProps> = ({
  ...
  // Load projects on mount and when refresh is triggered
  useEffect(() => {
    loadProjects();
  }, [loadProjects, refreshCounter.projects]);
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      // Check if it's a default project protection error
      if (errorMessage.toLowerCase().includes("default") || errorMessage.toLowerCase().includes("protected")) {
        toast.error("Cannot delete system projects (Chore and Repeatable)");
      } else {
        toast.error(errorMessage);
      }
      // Close dialog on error since we've shown the toast
      setDeletingProject(null);
    } finally {
      setIsDeleting(false);
    }
  }, [deletingProject, dataService, loadProjects, toast]);

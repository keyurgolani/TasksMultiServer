...
export const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  // Hide delete button for default projects (Chore, Repeatable)
  const canDelete = onDelete && !project.isDefault;
  const showActionButtons = !isMinimal && isHovered && (onEdit || canDelete);
  ...
  return (
    ...
    >
      ...
      {/* Project Name with Default Badge - adapts typography based on width */}
      <div className={cn("flex items-center gap-2", isCompact ? "" : "pr-16")}>
        <Typography
          variant={isCompact ? "body-sm" : "h5"}
          color="primary"
          className={cn(
            isCompact ? "truncate font-medium" : "line-clamp-2"
          )}
          data-testid="project-card-name"
        >
          {project.name}
        ...
        {project.isDefault && (
          <Badge
            variant="info"
            size="sm"
            data-testid="project-card-default-badge"
          >
            System
          </Badge>
        )}
